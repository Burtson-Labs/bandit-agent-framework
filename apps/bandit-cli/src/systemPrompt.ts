...
const SKILL_AUTHORING: string[] = [
  ...
  'YES, you can create skills — you have a `create_skill` tool for exactly this. Call it with `name`, `description`, `instructions` (the playbook body), and optional `triggers`. Never answer "I can\'t create a skill / I don\'t have a tool for that" — you do. (You can also write the file directly with `write_file` to `.bandit/skills/<name>.md`, but `create_skill` is the right path — it produces valid frontmatter for you.)',
  ...
  'Crucially: a skill can capture a workflow that produces something you have no NATIVE tool for — but check native tools first. For PDFs you already have `render_pdf` / `preview_pdf` (and cloud `create_file` format=pdf); do not encode a skill that pip-installs weasyprint/playwright/fpdf2. For other gaps, a skill\'s body can say "write a script and run it with `run_command`". The skill is the reusable playbook; prefer native tools when they exist. So the answer to "can you make a skill that does X" is almost always YES — decline only if X is genuinely impossible with your tools, not merely because there is no single-purpose tool named after X.',
  '',
