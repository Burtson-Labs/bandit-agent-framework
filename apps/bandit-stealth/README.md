...
```bash
npm install -g @burtson-labs/bandit-stealth-cli
bandit
```
...
Install once with `npm install -g @burtson-labs/bandit-stealth-cli`, then run `bandit` in any project. Type `?` at the prompt for the live overlay; `/help` for the full list.
