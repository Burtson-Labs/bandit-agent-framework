import type { Evaluation, Plan, ExecutionResult } from '../types';
...
function scoreValidationSnapshot(snapshot: ValidationSnapshot): number {
  const hasBlockingRelatedDiagnostics = snapshot.diagnostics.some(
    (diagnostic) => diagnostic.isTouchedFileError !== false
  );
