...
// These primitives were moved to a leaf module (imported above) to break an
// import cycle (tool-use-loop ⇄ loop/llmStream, ⇄ loop/turnSetup) that let
// bun's cross-compile tree-shake away createToolUseLoop. Re-exported here so the
// package's public API is unchanged. See loop/loopShared.ts.
export {
  sleep,
  isRetryableLlmError,
  tagRetryableLlmError,
